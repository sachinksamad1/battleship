...
export function getNeighbors(coord: Coordinate): Coordinate[] {
	...
	return neighbors.filter((c) => c.x >= 0 && c.x < BOARD_SIZE && c.y >= 0 && c.y < BOARD_SIZE);
}
...
function createProbabilityMap(board: Board, shipLengths: number[]): number[][] {
	const map: number[][] = Array(BOARD_SIZE)
		.fill(0)
...
function createMonteCarloMap(board: Board, shipDefinitions: ShipDefinition[]): number[][] {
	...
	for (let i = 0; i < simulations; i++) {
		const simOccupied = Array(BOARD_SIZE)
			.fill(0)
	...
	if (successfulSimulations < 10) {
		return createProbabilityMap(
			board,
			shipDefinitions.map((s) => s.length)
		);
	}
 ...
 */
export function selectAiMove(board: Board, difficulty: AIDifficulty, aiState: AIState): Coordinate {
	...
	if (difficulty === 'expert') {
		...
		probabilityMap = createProbabilityMap(
			board,
			remainingShips.map((s) => s.length)
		);
	}
...
export function generateAiFleet(difficulty: AIDifficulty): { board: Board; ships: ShipState[] } {
	...
	const attemptPlacement = (
		strategy: AIDifficulty
	): { board: Board; ships: ShipState[] } | null => {
			...
			while (!placed && attempts < MAX_ATTEMPTS) {
				...
				if (strategy !== 'easy') {
					...
					if (!isDecoyAttempt) {
						...
						if (!touches)
							spacingViolated = true; // Reject if it doesn't touch
	...
	// Fallback / standard
	return (
		attemptPlacement(difficulty) || attemptPlacement('easy') || { board: createBoard(), ships: [] }
	);
}
