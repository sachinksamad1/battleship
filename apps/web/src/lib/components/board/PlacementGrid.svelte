...
<div
	class="grid w-full max-w-[500px] grid-cols-10 gap-0 overflow-hidden rounded-xl border-2 border-blue-900 bg-blue-900 shadow-2xl"
>
	{#each Array.from({ length: BOARD_SIZE }, (_, i) => i) as y (y)}
		{#each Array.from({ length: BOARD_SIZE }, (_, i) => i) as x (x)}
			...
			{@const isPreview =
				...
				($placementStore.currentOrientation === 'H'
					? y === hoveredCoord.y &&
						x >= hoveredCoord.x &&
						x < hoveredCoord.x + $placementStore.selectedShip.length
					: x === hoveredCoord.x &&
						y >= hoveredCoord.y &&
						y < hoveredCoord.y + $placementStore.selectedShip.length)}
			...
			<div
               ...
               cursor-crosshair hover:bg-blue-800/20"
				onclick={() => handleCellClick(x, y)}
				onmouseenter={() => onMouseEnter(x, y)}
				onmouseleave={onMouseLeave}
			>
				{#if cell.occupied}
					<div class="h-full w-full border border-white/10 bg-slate-500/20 shadow-inner"></div>
				{/if}
			</div>
		{/each}
	{/each}
